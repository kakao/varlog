...
func (r *runtime) registerMemStats() error {
	var (
		...
		heapAlloc    metric.Float64UpDownSumObserver
		heapIdle     metric.Float64UpDownSumObserver
		heapInuse    metric.Float64UpDownSumObserver
		heapObjects  metric.Float64UpDownSumObserver
		heapReleased metric.Float64UpDownSumObserver
		heapSys      metric.Float64UpDownSumObserver
		liveObjects  metric.Float64UpDownSumObserver

		// TODO: is ptrLookups useful? I've not seen a value
		// other than zero.
		ptrLookups metric.Float64SumObserver

		gcCount      metric.Float64SumObserver
		pauseTotalNs metric.Float64SumObserver
	...
	batchObserver := r.meter.NewBatchObserver(func(ctx context.Context, result metric.BatchObserverResult) {
		...
		result.Observe(
			nil,
			heapAlloc.Observation(float64(memStats.HeapAlloc)),
			heapIdle.Observation(float64(memStats.HeapIdle)),
			heapInuse.Observation(float64(memStats.HeapInuse)),
			heapObjects.Observation(float64(memStats.HeapObjects)),
			heapReleased.Observation(float64(memStats.HeapReleased)),
			heapSys.Observation(float64(memStats.HeapSys)),
			liveObjects.Observation(float64(memStats.Mallocs-memStats.Frees)),
			ptrLookups.Observation(float64(memStats.Lookups)),
			gcCount.Observation(float64(memStats.NumGC)),
			pauseTotalNs.Observation(float64(memStats.PauseTotalNs)),
		)
	...
	if heapAlloc, err = batchObserver.NewFloat64UpDownSumObserver(
	...
	if heapIdle, err = batchObserver.NewFloat64UpDownSumObserver(
	...
	if heapInuse, err = batchObserver.NewFloat64UpDownSumObserver(
	...
	if heapObjects, err = batchObserver.NewFloat64UpDownSumObserver(
	...
	if heapReleased, err = batchObserver.NewFloat64UpDownSumObserver(
	...
	if heapSys, err = batchObserver.NewFloat64UpDownSumObserver(
	...
	if ptrLookups, err = batchObserver.NewFloat64SumObserver(
	...
	if liveObjects, err = batchObserver.NewFloat64UpDownSumObserver(
	...
	if gcCount, err = batchObserver.NewFloat64SumObserver(
		"runtime.go.gc.count",
	...
	if pauseTotalNs, err = batchObserver.NewFloat64SumObserver(
