...
func (h *host) register() error {
	var (
		...
		hostMemoryUsage       metric.Float64UpDownSumObserver
		hostMemoryUtilization metric.Float64UpDownSumObserver

		networkIOUsage metric.Float64SumObserver
	...
	batchObserver := h.meter.NewBatchObserver(func(ctx context.Context, result metric.BatchObserverResult) {
		...
		// Host memory usage
		result.Observe(AttributeMemoryUsed, hostMemoryUsage.Observation(float64(vmStats.Used)))
		result.Observe(AttributeMemoryAvailable, hostMemoryUsage.Observation(float64(vmStats.Available)))
		...
		result.Observe(AttributeNetworkTransmit, networkIOUsage.Observation(float64(ioStats[0].BytesSent)))
		result.Observe(AttributeNetworkReceive, networkIOUsage.Observation(float64(ioStats[0].BytesRecv)))
	})
	...
	if hostMemoryUsage, err = batchObserver.NewFloat64UpDownSumObserver(
		"system.memory.usage",
	...
	if networkIOUsage, err = batchObserver.NewFloat64SumObserver(
		"system.network.io",
		metric.WithUnit(unit.Bytes),
		metric.WithDescription(
