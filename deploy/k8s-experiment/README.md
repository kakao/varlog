...
agent는 daemonset으로 설정한다. 같은 노드의 다른 pod 에서 접근하기 위해 hostNetwork를 이용한다. hostNetwork의 경우, dnsPolicy를 ClusterFirstWithHostNet
...
storagenode도 마찬가지로 host network를 사용한다. 그러므로 외부 노출을 위한 tcp-services는 필요가 없다.
