...
fill은 CR의 반대방향으로 진행한다. fill은 기존에 해당 로그 위치에 데이터가
있다면 overwrite할 수 있는 기능을 갖고 있다(force=true). CR의 마지막인 SN3에
write 시도를 하는 경우, overwrite를 하지 않는다. 만약 err_written 을 돌려 받고
그 위치에 쓰인 데이터가 Junk가 아니라면 정상적인 append가 이루어진것이다. 하지만
Junk값이 쓰여져 있다면 다른 클라이언트가 먼저 fill한 것이므로, 더 이상 fill을
진행하지 않는다. (만약 다른 클라이언트의 failure를 걱정해야 한다면 계속해서
fill을 진행할수도 있다.)
