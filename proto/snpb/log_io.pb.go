...
func (m *AppendRequest) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 3:
			...
			if len(m.Payload) < cap(m.Payload) {
				m.Payload = m.Payload[:len(m.Payload)+1]
			} else {
				m.Payload = append(m.Payload, nil)
			}
			lastIndex := len(m.Payload) - 1
			m.Payload[lastIndex] = slices.Grow(m.Payload[lastIndex], postIndex-iNdEx)
			m.Payload[lastIndex] = m.Payload[lastIndex][:postIndex-iNdEx]
			copy(m.Payload[lastIndex], dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
